            ...
            <div className="container mx-auto px-4 lg:px-6 relative z-10">
               <div className="grid lg:grid-cols-2 gap-12 items-center">
                  <FadeIn direction="right">
                     <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gsb-blue/10 text-gsb-blue text-sm font-bold mb-6 border border-gsb-blue/20">
                     ...
                     <div className="flex flex-wrap gap-4">
                        <Button size="lg" className="bg-gsb-orange text-white hover:bg-gsb-orange/90 font-bold rounded-full px-8 h-14 text-lg shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all" asChild>
                           <Link href="#lokasi">Lihat Lokasi</Link>
                        </Button>
                        <Button size="lg" variant="outline" className="border-2 border-gsb-maroon text-gsb-maroon hover:bg-gsb-maroon hover:text-white font-bold rounded-full px-8 h-14 text-lg" asChild>
                  ...
                  <FadeIn direction="left" className="relative hidden lg:block">
                     <div className="relative z-10 bg-card border border-border p-8 rounded-3xl shadow-xl transform rotate-3 hover:rotate-0 transition-transform duration-500">
                        ...
                        <div className="space-y-4">
                           {[1, 2, 3, 4].map((i) => (
                              <div key={i} className="h-2 bg-muted rounded-full w-full overflow-hidden">
                                 <div className="h-full bg-gsb-orange w-[70%] rounded-full" style={{ width: `${60 + i * 10}%` }} />
                              </div>
                           ))}
                        </div>
                     </div>
                  ...
                  {[
                     {
                        ...
                        level: "PAUD - SMA",
                        color: "border-gsb-blue/30 hover:border-gsb-blue"
                     },
                     {
                        ...
                        time: "10.00 - 12.00 WIB",
                        level: "PAUD - SMP",
                        color: "border-gsb-tosca/30 hover:border-gsb-tosca"
                     },
                     {
                        place: "Zoom Meeting",
                        city: "Online",
                        time: "Fleksibel",
                        level: "Nasional",
                        color: "border-gsb-yellow/30 hover:border-gsb-yellow"
                     }
                  ].map((loc, idx) => (
                     <FadeIn key={loc.place} delay={idx * 0.1} className={`bg-background p-8 rounded-[2rem] border shadow-sm hover:shadow-xl transition-all group ${loc.color}`}>
               ...
               <div className="grid lg:grid-cols-2 gap-16 items-center">
                  <FadeIn className="order-2 lg:order-1">
                     <div className="grid gap-4">
                        {curriculum.map((item) => (
                           <div key={item.week} className="bg-background p-6 rounded-2xl border border-border flex gap-4 items-center hover:border-gsb-orange transition-colors shadow-sm">
